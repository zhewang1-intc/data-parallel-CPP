...
template <SIGN_INT_TYPE S4_T> inline int8_t get_s8(int8_t v) {
  switch (S4_T) {
  case S4_CLIP:
    return v << 4;
  case S4_FULLRANGE:
...
// [[intel::device_indirectly_callable]] SYCL_EXTERNAL void __regcall
// invoke_sub_dq(simd<int8_t, 16> slm_vec) SYCL_ESIMD_FUNCTION {}
    ...
    h.parallel_for(nd_range{global, local},
                   [=](nd_item<2> it) [[intel::sycl_explicit_simd]] {
                     ...
                     if (dq_j + 64 <= n && i < k) {
                       ...
                       simd<int8_t, 16> ttt =
                           slm_block_load<int8_t, 16>(test.get_offset());
                       ...
                       auto vec_fp32_cp =
                           convert<float, int8_t, 32>(bit4_vec_cp);
...
void gpu_dequant(queue &q, int8_t *src, DST_T *dst, float *scale, int k, int n,
  ...
  for (int i = 0; i < k; i += KTILE) {
    for (int j = 0; j < n; j += NTILE) {
      ...
      esimd_gpu_dequant_s4fullrange_f32_KxN<KTILE, NTILE / 64, LOCAL_K,
                                            LOCAL_N>(q, src, dst, scale, k, n,
                                                     blksize, i, j);
    }
  }
  q.wait();
...
template <typename DST_T> void dump_matrix(DST_T *mat, int k, int n) {
  for (int i = 0; i < k; i++) {
    for (int j = 0; j < n; j++) {
      std::cout << mat[i * n + j] << " ";
    }
...
void ut(int K, int N, int blksize) {
  ...
  for (int i = 0; i < cpu_dq.size(); i++) {
    ...
    if (cpu_dq[i] != gpu_dq[i]) {
      std::cout << "wrong idx: " << i << " " << cpu_dq[i] << "vs" << gpu_dq[i]
                << std::endl;
      ok = false;
      // break;
    }
...
int main() {
  ...
  std::cout << "The device has " << local_mem_size
