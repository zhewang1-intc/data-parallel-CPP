...
void gpu_dequant_s4fullrange_f32_KxN(queue &q, int8_t *src, DST_T *dst,
  ...
  q.submit([&](handler &h) {
    ...
    h.parallel_for(nd_range{global, local}, [=](nd_item<2> it) {
      ...
      int fp32_j = s4_j * 2;
      if (i < k && fp32_j + 1 < n) {
        // int8_t* cur_src=src+i*n/2+s4_j;
        // float* scale_ptr=scale+i/blksize*n+fp32_j;
        // auto vec_reg=block_load<int8_t,32>(cur_src);
        // auto vec_reg_cp=vec_reg.select<32,1>(0);
        // auto vec_scale=block_load<float,32>(scale_ptr);
        // vec_reg&=0x0f;
        // vec_reg_cp=(vec_reg_cp>>4)&0x0f;
        // auto vec_reg_f32=convert<float,int8_t,32>(vec_reg);
        // auto
        // vec_reg_cp_fp32=convert<float,int8_t,32>(vec_reg_cp);
        // vec_reg_f32*=vec_scale;
        // vec_reg_cp_fp32*=vec_scale;
        // float* dst_ptr=dst+i*n+fp32_j;
        // block_store<float,32>(dst_ptr,vec_scale);

        // sycl::ext::oneapi::experimental::printf("i: %d,j:
        // %d\n", i, s4_j);
